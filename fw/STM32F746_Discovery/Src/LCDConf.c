...
void LCD_X_Config(void) 
{
	...
	HAL_GPIO_WritePin(GPIOI, GPIO_PIN_12, GPIO_PIN_SET);		// Display enable
	HAL_GPIO_WritePin(GPIOK, GPIO_PIN_3,  GPIO_PIN_SET);		// Display LED On
	...
	GUI_DEVICE_CreateAndLink(DISPLAY_DRIVER_0, COLOR_CONVERSION_0, 0, 0);					// Set display driver and color conversion for 1st layer
	...
	GUI_DEVICE_CreateAndLink(DISPLAY_DRIVER_1, COLOR_CONVERSION_1, 0, 1);		// Set display driver and color conversion for 2nd layer
...
#if (USE_TOUCH == 1)
	GUI_TOUCH_SetOrientation(DISPLAY_ORIENTATION);// Set orientation of touch screen
