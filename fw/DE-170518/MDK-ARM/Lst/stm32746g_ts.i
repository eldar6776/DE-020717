# 1 "../Drivers/BSP/STM32F746/stm32746g_ts.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 370 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Drivers/BSP/STM32F746/stm32746g_ts.c" 2
# 75 "../Drivers/BSP/STM32F746/stm32746g_ts.c"
# 1 "../Drivers/BSP/STM32F746/stm32746g_ts.h" 1
# 46 "../Drivers/BSP/STM32F746/stm32746g_ts.h"
...
# 47 "../Drivers/BSP/STM32F746/stm32746g_ts.h" 2
...
# 76 "../Drivers/BSP/STM32F746/stm32746g_ts.c" 2
# 120 "../Drivers/BSP/STM32F746/stm32746g_ts.c"
static TS_DrvTypeDef *tsDriver;
static uint16_t tsXBoundary, tsYBoundary;
static uint8_t tsOrientation;
static uint8_t I2cAddress;
# 146 "../Drivers/BSP/STM32F746/stm32746g_ts.c"
uint8_t TOUCH_SCREEN_Init(uint16_t ts_SizeX, uint16_t ts_SizeY)
{
 ...
 ft5336_ts_drv.Init(((uint16_t)0x70));
 if(ft5336_ts_drv.ReadID(((uint16_t)0x70)) == ((uint8_t)0x79))
 {

  tsDriver = &ft5336_ts_drv;
  I2cAddress = ((uint16_t)0x70);
  tsOrientation = ((uint8_t) 0x01);
...
# 185 "../Drivers/BSP/STM32F746/stm32746g_ts.c"
 return TS_OK;
}
...
uint8_t BSP_TS_GetState(TS_StateTypeDef *TS_State)
{
  static uint32_t _x[((uint32_t) ((uint8_t)0x05))] = {0, 0};
  static uint32_t _y[((uint32_t) ((uint8_t)0x05))] = {0, 0};
  uint8_t ts_status = TS_OK;
  uint16_t x[((uint32_t) ((uint8_t)0x05))];
  uint16_t y[((uint32_t) ((uint8_t)0x05))];
  uint16_t brute_x[((uint32_t) ((uint8_t)0x05))];
  uint16_t brute_y[((uint32_t) ((uint8_t)0x05))];
  uint16_t x_diff;
  uint16_t y_diff;
  uint32_t index;
  ...
  if(TS_State->touchDetected)
  {
    for(index=0; index < TS_State->touchDetected; index++)
    {
      ...
      if(tsOrientation == ((uint8_t) 0x01))
      {
        x[index] = brute_x[index];
        y[index] = brute_y[index];
      }

      if(tsOrientation & ((uint8_t) 0x02))
      {
        x[index] = 4096 - brute_x[index];
      }

      if(tsOrientation & ((uint8_t) 0x04))
      {
        y[index] = 4096 - brute_y[index];
      }

      if(tsOrientation & ((uint8_t) 0x08))
      {
...
# 397 "../Drivers/BSP/STM32F746/stm32746g_ts.c"
uint8_t BSP_TS_ResetTouchData(TS_StateTypeDef *TS_State)
{
  uint8_t ts_status = TS_ERROR;
  uint32_t index;

  if (TS_State != (TS_StateTypeDef *)0)
  {
    TS_State->gestureId = GEST_ID_NO_GESTURE;
    TS_State->touchDetected = 0;

    for(index = 0; index < ((uint32_t) ((uint8_t)0x05)); index++)
    {
