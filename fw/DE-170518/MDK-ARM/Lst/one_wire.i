...
volatile uint32_t onewire_timer;
 volatile uint32_t onewire_timeout_timer;
uint32_t onewire_flags;
uint8_t onewire_buffer[96];
...
void ONEWIRE_Init(void)
{
 ...
 {
  (onewire_flags &= 0xfffffffd);
  ONEWIRE_SetUsart(((uint32_t)9600));
...
void ONEWIRE_Service(void)
{
 ...
 else if((onewire_flags & 0x00000002))
 {
 ...
 else
 {
  if (OnewireState == ONEWIRE_PACKET_RECEIVED)
  {
   ...
   if(onewire_buffer[2] == ((uint8_t)0x06))
   {
    BUTTON_SetNewState(0x803, onewire_buffer[3]);
    BUTTON_SetNewState(0x804, onewire_buffer[4]);
    BUTTON_SetNewState(0x805, onewire_buffer[5]);
   }



   if(onewire_buffer[6] == ((uint8_t)0x06))
   {
   ...
   if(onewire_buffer[14] == ((uint8_t)0x06))
   {
   ...
   if(onewire_buffer[27] == ((uint8_t)0x06))
   {
# 276 "../Src/one_wire.c"
   }
   ...
   if(onewire_buffer[56] == ((uint8_t)0x06))
   {
    if(onewire_buffer[57] == 0) (display_flags &= (~ (1 << 0)));
    else (display_flags |= (1 << 0));

    if(onewire_buffer[58] == 0) (display_flags &= (~ (1 << 1)));
    else (display_flags |= (1 << 1));
   }
  ...
  {
   ClearBuffer(onewire_buffer, 96);



   onewire_buffer[0] = 0x22;
   onewire_buffer[1] = 0x11;



   onewire_buffer[2] = ((uint8_t)0x15);
   onewire_buffer[3] = BUTTON_GetState(0x803);
   onewire_buffer[4] = BUTTON_GetState(0x804);
   onewire_buffer[5] = BUTTON_GetState(0x805);



   onewire_buffer[6] = ((uint8_t)0x15);
   onewire_buffer[7] = date.Date;
   onewire_buffer[8] = date.WeekDay;
   onewire_buffer[9] = date.Month;
   onewire_buffer[10] = date.Year;
   onewire_buffer[11] = time.Hours;
   onewire_buffer[12] = time.Minutes;
   onewire_buffer[13] = time.Seconds;



   onewire_buffer[14] = ((uint8_t)0x15);
   onewire_buffer[15] = Thermostat_1.valve;
   onewire_buffer[16] = Thermostat_1.fan_mode;
   onewire_buffer[17] = Thermostat_1.fan_speed;
   onewire_buffer[18] = Thermostat_1.ctrl_mode;
   onewire_buffer[19] = (Thermostat_1.actual_temperature >> 8);
   onewire_buffer[20] = (Thermostat_1.actual_temperature & 0xff);
   onewire_buffer[21] = (Thermostat_1.set_temperature >> 8);
   onewire_buffer[22] = (Thermostat_1.set_temperature & 0xff);
   onewire_buffer[23] = Thermostat_1.set_temperature_diff;
   onewire_buffer[24] = Thermostat_1.fan_low_speed_band;
   onewire_buffer[25] = Thermostat_1.fan_middle_speed_band;
   onewire_buffer[26] = Thermostat_1.fan_speed_diff;



   onewire_buffer[27] = ((uint8_t)0x15);
# 369 "../Src/one_wire.c"
   onewire_buffer[56] = ((uint8_t)0x15);
   onewire_buffer[57] = ((display_flags & (1 << 0)) != 0);
   onewire_buffer[58] = ((display_flags & (1 << 1)) != 0);



   onewire_buffer[59] = current_outdoor_temperature;
   onewire_buffer[60] = 0x00;
   onewire_buffer[61] = 0x00;
   onewire_buffer[62] = 0x00;
   onewire_buffer[63] = CalcCRC(onewire_buffer, 63);
   HAL_Delay(10);
   HAL_HalfDuplex_EnableTransmitter(&huart2);
   HAL_UART_Transmit(&huart2, onewire_buffer, 64, 345);
...
uint8_t ONEWIRE_Reset(void)
{
 ...
 HAL_UART_Transmit(&huart2, onewire_buffer, 1, 800);
 HAL_UART_Receive(&huart2, onewire_buffer, 1, 800);
 ONEWIRE_SetUsart(((uint32_t)115200));
...
void ONEWIRE_SetUsart(uint32_t setup)
{
 ...
 huart2.Init.WordLength = ((uint32_t)0x0000U);
...
uint8_t ONEWIRE_ReceiveBit(void)
{
 uint8_t txd, rxd;
 txd = 0xff;
 HAL_UART_Transmit(&huart2, &txd, 1, 800);
 HAL_UART_Receive(&huart2, &rxd, 1, 800);
...
void ONEWIRE_SendBit(uint8_t send_bit)
{
 ...
 HAL_UART_Transmit(&huart2, &txb, 1, 800);
 HAL_UART_Receive(&huart2, &rxb, 1, 800);
}
...
void ONEWIRE_Send(uint8_t *command, uint8_t lenght)
{
 ...
 HAL_UART_Transmit(&huart2, onewire_buffer, one_wire_lenght, 800);
}
...
void ONEWIRE_Receive(uint8_t *data, uint8_t lenght)
{
 ...
 {
  HAL_UART_Transmit(&huart2, &tx_byte, 1, 800);
  HAL_UART_Receive(&huart2, &onewire_buffer[i], 1, 800);
 }
