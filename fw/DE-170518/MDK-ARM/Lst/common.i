...
void Str2Hex(uint8_t *p_str, uint16_t lenght, uint8_t *p_hex)
{
 ...
 while(lenght)
 {
  if((p_str[str_cnt] - 48) > 9) p_hex[hex_cnt] = ((((p_str[str_cnt]) >= 'A') && ((p_str[str_cnt]) <= 'F')) ? ((p_str[str_cnt]) - 'A'+10) : ((p_str[str_cnt]) - 'a'+10)) << 4;
  else p_hex[hex_cnt] = (p_str[str_cnt] - '0') << 4;
  ++str_cnt;
  if((p_str[str_cnt] - 48) > 9) p_hex[hex_cnt] += ((((p_str[str_cnt]) >= 'A') && ((p_str[str_cnt]) <= 'F')) ? ((p_str[str_cnt]) - 'A'+10) : ((p_str[str_cnt]) - 'a'+10));
  else p_hex[hex_cnt] += (p_str[str_cnt] - '0');
  ++str_cnt;
  ++hex_cnt;
  --lenght;
 }
...
void Hex2Str(uint8_t *p_hex, uint16_t lenght, uint8_t *p_str)
{
 ...
 while(lenght)
 {
  if((p_hex[hex_cnt] >> 4) > 9) p_str[str_cnt] = (p_hex[hex_cnt] >> 4) + 55;
  else p_str[str_cnt] = (p_hex[hex_cnt] >> 4) + 48;
  ++str_cnt;
  if((p_hex[hex_cnt] & 0x0f) > 9) p_str[str_cnt] = (p_hex[hex_cnt] & 0x0f) + 55;
...
uint32_t Str2Int(uint8_t *p_inputstr, uint32_t *p_intnum)
{
  ...
  if ((p_inputstr[0] == '0') && ((p_inputstr[1] == 'x') || (p_inputstr[1] == 'X')))
  {
    ...
    {
      if (((((p_inputstr[i]) >= 'A') && ((p_inputstr[i]) <= 'F')) || (((p_inputstr[i]) >= 'a') && ((p_inputstr[i]) <= 'f')) || (((p_inputstr[i]) >= '0') && ((p_inputstr[i]) <= '9'))))
      {
        val = (val << 4) + ((((p_inputstr[i]) >= '0') && ((p_inputstr[i]) <= '9')) ? ((p_inputstr[i]) - '0') : ((((p_inputstr[i]) >= 'A') && ((p_inputstr[i]) <= 'F')) ? ((p_inputstr[i]) - 'A'+10) : ((p_inputstr[i]) - 'a'+10)));
      }
      else
      {
  ...
  else
  {
    while ( ( i < 11 ) && ( res != 1 ) )
    {
      ...
      else if ((((p_inputstr[i]) >= '0') && ((p_inputstr[i]) <= '9')))
      {
        val = val * 10 + (p_inputstr[i] - '0');
      }
      else
      {
